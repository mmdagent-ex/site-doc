...
ファイルパスを指定することで指定されたファイルへログをすべて記録する。デフォルトは空白（Linux/Mac では標準出力へ出力、Windows では出力しない）。

{{<mdf>}}
log_file=
{{</mdf>}}

メッセージを標準入出力で受け渡しするかどうかを設定。デフォルトは `false` 。 `true` にすると、メッセージが標準出力へ出力され、かつ標準入力からの入力内容をそのままメッセージとして受け付ける。これを `true` にするとログが標準出力へ出力されなくなるためログを記録したい場合は `log_file` で保存先ファイルを指定すること。
