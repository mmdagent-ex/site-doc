...
接続直後に以下のメッセージが発行されます。このメッセージは外部プログラムに送られるので、外部プログラムはこのメッセージをキャッチすることで接続が確立したことを知ることができます。

{{<message>}}
REMOTE_EVENT_CONNECTED|接続先情報
{{</message>}}

なお接続が切断されたときには以下のメッセージが発行されます。このメッセージがMMDAgent-EX内で発行されるのは接続が切れたあとなので切断された外部プログラム側には送られませんが、MMDAgent-EX 側でこれを使って接続断を検知することが可能です。

{{<message>}}
REMOTE_EVENT_DISCONNECTED|接続先情報
{{</message>}}
...
この **AVATAR_EVENT_IDLE|START** が出るまでの待ち時間のデフォルトは15秒ですが、`Plugin_Remote_Idle_Timeout_Second=` を .mdf に記述することで秒数を変更できます。
...
アバターの頭部の目標とする姿勢情報を送信します。このメッセージが送信されるたび、MMDAgent-EXは対象モデルの頭部と上半身のボーンの姿勢をそれに合わせてセットします。連続で送信することでリアルタイムのヘッドトラッキングが行えます。ここで指定する移動量や回転量は目標姿勢の指定であり、実際の行動量ではない点に注意してください。例えば、30度回転のメッセージを2回連続で送信した場合、結果として60度の回転を行うのではなく、正面姿勢から30度の位置へ向きなさい、という指定になります。
...
Action Unit は表情筋の動きをエンコードするものであり、その値はモデルの表情モーフとは一対一に対応しません。__AV_AU で送られる各 Action Unit の値に対して、実際にモデルのどのモーフをどのようにセットするかを、shapemap ファイルで定義する必要があります。以下はAU番号6 (cheek raiser) を笑った目のモーフである「笑い」モーフへ、AU番号1 (inner brow raiser) を眉を挙げるモーフ「上」へ、AU番号4 (brow lowerer) を、眉を困った形にする「困る」モーフへマッピングする例です。具体的な記法は [.shapemap](../shapemap) を見てください。
...
任意のボーンを外部から制御します。これによるボーン制御では、自動補正は行われません。
...
このメッセージで指定する制御名 **name** と、実際に操作するモデル上のボーン名の対応は、shapemap 内で定義します。詳しくは [.shapemap](../shapemap) のページを見てください。
...
任意のモーフを外部から制御します。これによる制御では自動補正は行われません。

- **name**: 制御名
- **rate**: モーフ値（0.0～1.0）

このメッセージで指定する制御名 **name** と、実際に操作するモデル上のモーフ名の対応は shapemap 内で定義します。詳しくは [.shapemap](../shapemap) のページを見てください。
