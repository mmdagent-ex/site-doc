...
音声波形データをソケット経由で流し込み、リップシンク付きで再生させます。トラッキング制御中でなくても動作します。
...
音声伝送時の伝送音声からの自動リップシンクを行わない場面の指定を行います。これはフェイシャルトラッキングによる口形の動きと音声からのリップシンクが重複する際にどちらを選ぶかの設定です。デフォルトは `NO` 、つまりリップシンクを常に行います。
