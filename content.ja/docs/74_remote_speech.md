...
その後、オーディオデータを短いチャンクごとに区切って送信します。１つのチャンクは、以下のように文字列 "`SND`" に続けて4桁(10進)のチャンク長（バイト数）、それに続けてその指定したチャンク長（バイト数）分の音声波形データを繋げます。改行コードは不要です。これを音声の短いチャンク（1024バイト程度）ごとに連続して送ります。**チャンクのサイズは 4K Bytes を越えないようにしてください。** 長い音声データはチャンクごとに刻んで順に送り込んでください。
...
**送信終了後にすぐにソケットを close しないでください。** オーディオデータはMMDAgent-EX側でバッファリングされ、別スレッドで順次再生されます。そのため、送信は再生を待たずにすぐに終了します。送信終了後にソケット接続をすぐ close した場合、その時点で音声再生も含めて終了するため、オーディオが最後まで再生されません。送信後に接続を close したい場合は、その音声の秒数分だけ待ってから close するようにしてください。
