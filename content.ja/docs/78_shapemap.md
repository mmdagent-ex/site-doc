...
自動リップシンクや外部APIが出力する信号は同じですが、CGモデルの構造や部位の構成はモデルごとに大きく異なるため、MMDAgent-EXではこのようにモデルごとに対応を定義する仕様になっています。

## ファイル設置場所

Shapemapファイル (.shapemap) は、CGアバターのモデルファイル `xxx.pmd` が置いてあるフォルダと同じフォルダに、 `xxx.pmd.shapemap` のようにモデルファイル名の後ろに `.shapemap` を付けたファイル名で置きます。 `xxx.shapemap` ではなく `xxx.pmd.shapemap` である点に注意してください。

アバターとしてCGエージェントを利用する場合、モデルごとに .shapemap を必ず用意してください。Shapemap ファイルがない場合、CG モデルは外部APIおよび音声の自動リップシンクに対して動作しません。

サンプルの Shapemap は CG-CA のものを参考にしてください。配布されている CG-CA には Shapemap ファイルが既に定義されており、多くの MMD モデルは似た構造を持つため、そのままでも基本的な部分はおおよそ動きます。新たなCGモデルをセットアップする場合は、まず CG-CA に付属の .shapemap をコピーして始めるのがよいでしょう。
